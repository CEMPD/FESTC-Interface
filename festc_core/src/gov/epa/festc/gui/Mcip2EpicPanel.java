...
public class Mcip2EpicPanel extends UtilFieldsPanel implements PlotEventListener {
	...
	private Action browseDirAction(final String name, final JTextField text) {
		return new AbstractAction("Browse...") {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int option = chooser.showDialog(Mcip2EpicPanel.this,
						"Select");
	...
	protected String writeRunScriptScript(
		...
		sb.append("setenv GRID_NAME  \"" + (gridName.getText() == null ? "" : gridName.getText().trim()) + "\"" + ls + ls);
		sb.append("#" + ls 
				+ "#Set MCIP data directory which containts daily MCIP files" + ls 
				+ "#Daily MCIP data files have to have names with METCRO2D*\"date\"" + ls
				+ "#The \"date\" can be in one of the format: YYYYMMDD, YYMMDD, YYYYDDD, YYDDD" + ls
				+ "#" + ls);
		
		sb.append("setenv DATA_DIR   \"" + mdataDir.getText() + "\"" + ls + ls);
		sb.append("#" + ls
				+ "#Set CMAQ output dry/wet deposition data directory which containts daily CMAQ files" + ls
				+ "#" + ls);
		...
		sb.append("#" + ls
				+ "# Set NetCDF output file to store computed EPIC site daily weather and N deposition data" + ls 
				+ "# Only set this variable when EPIC grids are the same as MCIP/CMAQ grids." + ls  
	...
	private String getScriptHeader() {
		...
		sb.append("#     1. read in EPIC site lat and long location data, project them into MCIP data projection and" + ls);
		sb.append("#        convert them into column and row in MCIP grids." + ls);
		sb.append("#     2. loop through each day MCIP data to extract daily weather variables for each EPIC site" + ls);
		sb.append("#        and to write them into each EPIC site daily weather file." + ls);   
		sb.append("#     3. created WXPMRUN.DAT file for all generated EPIC daily weather files in order to run WXPM3020.DRB program." + ls); 
		sb.append("#        The program computes monthly weather data to be named in monthly weather list file like WPM1US.DAT." + ls);
		sb.append("#" + ls);
	...
	public void projectLoaded() {
		...
		if ( fields != null  ){
			...
			try {
				...
				endDate.setValue(NumberFormat.getNumberInstance().parse(fields.getEnddate()==null? "":fields.getEnddate()));
	...
	public void saveProjectRequested() {
		...
		if ( ySize != null ) fields.setYcellSize(Float.parseFloat(ySize.getText() == null? "" : ySize.getValue()+""));
		if ( xmin != null ) fields.setXmin(Float.parseFloat(xmin.getText() == null? "" : xmin.getValue()+""));
		...
		if (startDate != null) fields.setStartdate(startDate.getValue()== null? "" : startDate.getText());
		if (endDate != null) fields.setEnddate(endDate.getValue()== null? "" : endDate.getText());
		
		if (depositionSel != null)  fields.setDepSelection((String)depositionSel.getSelectedItem());
		if (depositionDir != null)  fields.setCmaqDepsDir(depositionDir.getText());
		if (mdataDir != null) 	fields.setDatadir(mdataDir.getText());
