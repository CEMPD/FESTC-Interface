...
public class HelpWindowWithContents extends OKWindow {
	private static final long serialVersionUID = -3954828275072932197L;

	JEditorPane editorPaneLeft, editorPaneRight;

	public HelpWindowWithContents(Dialog dialog, String title, String doc,
			String doc2) {
		super(dialog, title, false);
		inits(doc, doc2);
	}

	public HelpWindowWithContents(Frame frame, String title, String doc,
			String doc2) {
			...
			public void hyperlinkUpdate(HyperlinkEvent e) {
				if (e.getEventType() == HyperlinkEvent.EventType.ACTIVATED) {
					...
					if (e.getDescription().startsWith("#")) {
						// move to the link
						editorPaneRight.scrollToReference(e.getDescription()
								.substring(1));
	...
	public static void showContents(Window frame, String title, String doc,
			String doc2) {
		if (window == null) {
			if (frame instanceof Dialog)
				window = new HelpWindowWithContents((Dialog) frame, title, doc,
						doc2);
			else
				window = new HelpWindowWithContents((Frame) frame, title, doc,
						doc2);
		} else {
